...
def below_average_years
  ...
  Movie.where('score < 5').group('yr').select('COUNT(*) AS bad_movies, yr').order('yr DESC') 
...
def alphabetized_actors
  ...
  Actor.group('id').select('id, name').order('name ASC').limit(10)
...
def pulp_fiction_actors
  ...
  Movie.where("title = 'Pulp Fiction' ")
  .joins(:castings)
  .joins(:actors)
